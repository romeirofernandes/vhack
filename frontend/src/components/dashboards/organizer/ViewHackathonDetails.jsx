    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white/70 mb-6">The hackathon you're looking for doesn't exist or has been deleted.</p>
          <Button onClick={() => navigate('/dashboard')} className="bg-white text-zinc-950 hover:bg-white/90">
            Back to Dashboard
          </Button>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            ...
            <Card className="bg-zinc-900/40 border-zinc-800/60 backdrop-blur-sm">
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-white">{hackathon.participants?.length || 0}</div>
            ...
            <Card className="bg-zinc-900/40 border-zinc-800/60 backdrop-blur-sm">
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-white">{hackathon.judges?.length || 0}</div>
            ...
            <Card className="bg-zinc-900/40 border-zinc-800/60 backdrop-blur-sm">
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-white">
                  {getDaysLeft(hackathon.timelines?.hackathonStart || hackathon.startDate)}
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <>
                      <div className="flex justify-between items-center p-3 bg-white/5 rounded-lg">
                        <div>
                          ...
                          <p className="text-sm text-white/70">{formatDate(hackathon.timelines.registrationStart)}</p>
                        </div>
                        <Badge className="bg-green-500/20 text-green-300">Start</Badge>
                      </div>
                      
                      <div className="flex justify-between items-center p-3 bg-white/5 rounded-lg">
                        <div>
                          <p className="font-medium text-white">Registration Ends</p>
                          <p className="text-sm text-white/70">{formatDate(hackathon.timelines.registrationEnd)}</p>
                        </div>
                        <Badge className="bg-yellow-500/20 text-yellow-300">Deadline</Badge>
                      </div>

                      <div className="flex justify-between items-center p-3 bg-white/5 rounded-lg">
                        <div>
                          <p className="font-medium text-white">Hackathon Starts</p>
                          <p className="text-sm text-white/70">{formatDate(hackathon.timelines.hackathonStart)}</p>
                        </div>
                        <Badge className="bg-blue-500/20 text-blue-300">Event</Badge>
                      </div>

                      <div className="flex justify-between items-center p-3 bg-white/5 rounded-lg">
                        <div>
                          <p className="font-medium text-white">Hackathon Ends</p>
                          <p className="text-sm text-white/70">{formatDate(hackathon.timelines.hackathonEnd)}</p>
                        </div>
                        <Badge className="bg-red-500/20 text-red-300">End</Badge>
                      </div>

                      {hackathon.timelines.resultsDate && (
                        <div className="flex justify-between items-center p-3 bg-white/5 rounded-lg">
                          <div>
                            <p className="font-medium text-white">Results Announcement</p>
                            <p className="text-sm text-white/70">{formatDate(hackathon.timelines.resultsDate)}</p>
                          </div>
                          <Badge className="bg-purple-500/20 text-purple-300">Results</Badge>
                        </div>
                      )}
                    </>
                  )}
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        {hackathon.judges.map((judge) => (
                          <div key={judge._id} className="flex items-center space-x-3 p-3 bg-green-500/5 rounded-lg border border-green-500/20">
                            ...
                            <div className="flex-1">
                              <p className="font-medium text-white">{judge.displayName || 'Anonymous'}</p>
                              <p className="text-sm text-white/70">{judge.email}</p>
                            </div>
                            <Badge className="bg-green-500/20 text-green-300">
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        {hackathon.invitedJudges.map((judge) => (
                          <div key={judge._id} className="flex items-center space-x-3 p-3 bg-yellow-500/5 rounded-lg border border-yellow-500/20">
                            <Avatar className="w-10 h-10">
                              <AvatarImage src={judge.photoURL} />
            ...
            <div className="space-y-6">
                ...
                <Card className="bg-zinc-900/40 border-zinc-800/60 backdrop-blur-sm">
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex justify-between">
                      ...
                      <span className="text-white font-medium">{hackathon.teamSettings.minTeamSize || 1}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-white/70">Max Team Size:</span>
                      <span className="text-white font-medium">{hackathon.teamSettings.maxTeamSize || 4}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-white/70">Solo Allowed:</span>
                      <Badge className={hackathon.teamSettings.allowSolo ? "bg-green-500/20 text-green-300" : "bg-red-500/20 text-red-300"}>
              ...
              <Card className="bg-zinc-900/40 border-zinc-800/60 backdrop-blur-sm">
                ...
                <CardContent className="space-y-3">
                  <Button
                    onClick={() => navigate(`/organizer/hackathon/${hackathonId}/participants`)}
                    className="w-full bg-blue-500 hover:bg-blue-600"
                  >
                    <TbUsers className="w-4 h-4 mr-2" />
                    View Participants
                  </Button>
                  <Button
                    onClick={() => navigate(`/organizer/hackathon/${hackathonId}/submissions`)}
                  ...
                  <Button
                    onClick={() => navigate(`/organizer/hackathon/${hackathonId}/analytics`)}
                    variant="outline"
              ...
              <Card className="bg-zinc-900/40 border-zinc-800/60 backdrop-blur-sm">
                ...
                <CardContent className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="text-white text-sm">{new Date(hackathon.createdAt).toLocaleDateString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-white/70">Last Updated:</span>
                    <span className="text-white text-sm">{new Date(hackathon.updatedAt).toLocaleDateString()}</span>
                  </div>
                </CardContent>
              </Card>
            </div>
